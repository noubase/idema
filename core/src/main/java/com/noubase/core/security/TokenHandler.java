...
public final class TokenHandler<U extends ExpirableUserDetails> {
	...
	public U parseUserFromToken(@NotNull String token) {
		...
		if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
			try {
				...
				if (validHash) {
					final U user = fromJSON(userBytes);
	...
	@NotNull
	public String createTokenForUser(U user) {
		byte[] userBytes = toJSON(user);
		byte[] hash = createHmac(userBytes);
		return toBase64(userBytes) + SEPARATOR + toBase64(hash);
	}

	private U fromJSON(@NotNull final byte[] userBytes) {
		try {
